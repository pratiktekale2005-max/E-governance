...
export default function App() {
  const [settings, setSettings] = useState<AppSettings>({
    ...
    systemInstructions: 'You are Sahayak AI, a friendly Digital Government Officer. Your duty is to help users discover appropriate government scholarship schemes and welfare policies by conducting an eligibility questionnaire. Speak in a helpful and polite Indian government representative tone.',
  });

  useEffect(() => {
    const savedKey = localStorage.getItem('gemini_api_key') || '';
    const savedModel = localStorage.getItem('gemini_model') || 'gemini-3.6-flash';
    const savedPrompt = localStorage.getItem('gemini_system_prompt') || 'You are Sahayak AI, a friendly Digital Government Officer. Your duty is to help users discover appropriate government scholarship schemes and welfare policies by conducting an eligibility questionnaire. Speak in a helpful and polite Indian government representative tone.';
